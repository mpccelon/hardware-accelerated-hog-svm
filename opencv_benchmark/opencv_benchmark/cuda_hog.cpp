...
{
	cout << "Histogram of Oriented Gradients descriptor and detector sample. FOR COMPUTERS WITH CUDA ONLY\n"
		<< "\nUsage: hog\n"
		<< "  (<image>|--video <vide>|--camera <camera_id>) # frames source\n"
		<< "  or"
		<< "  (--folder <folder_path>) # load images from folder\n"
		<< "  [--svm <file> # load svm file (XML)"
...
App::App(const Args& s)
{
	...
	cout << "Win size: (" << args.win_width << ", " << args.win_width << ")\n";
...
void App::run()
{
	...
	Size win_size(args.win_width, args.win_width);
	...
	if (args.svm_load) {
		...
		cpu_hog.load(model_file_name);
		svm_model = cpu_hog.svmDetector;
		gpu_hog = cv::cuda::HOG::create(cpu_hog.winSize, cpu_hog.blockSize, cpu_hog.blockStride, cpu_hog.cellSize, cpu_hog.nbins);
		gpu_hog->setSVMDetector(svm_model);
	}
	else {
