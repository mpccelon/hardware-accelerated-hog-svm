...
#ifndef _xillybus_wrapper_lbW_H_
#define _xillybus_wrapper_lbW_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "xillybus_wrapper_lbW_memcore.h"

SC_MODULE(xillybus_wrapper_lbW) {
...
protected:
    ...
    xillybus_wrapper_lbW_memcore* xillybus_wrapper_lbW_memcore_U;
...
public:
    ...
    SC_CTOR(xillybus_wrapper_lbW) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        xillybus_wrapper_lbW_memcore_U = new xillybus_wrapper_lbW_memcore("xillybus_wrapper_lbW_memcore_U");
        xillybus_wrapper_lbW_memcore_U->clk      ( clk );
        xillybus_wrapper_lbW_memcore_U->reset    ( reset );
        xillybus_wrapper_lbW_memcore_U->ce0      ( i_ce0 );
        xillybus_wrapper_lbW_memcore_U->we0      ( i_we0 );
        xillybus_wrapper_lbW_memcore_U->address0 ( memcore_iaddr );
        xillybus_wrapper_lbW_memcore_U->d0       ( i_d0 );
        xillybus_wrapper_lbW_memcore_U->q0       ( i_q0 );

        xillybus_wrapper_lbW_memcore_U->ce1      ( t_ce0 );
        xillybus_wrapper_lbW_memcore_U->we1      ( t_we0 );
        xillybus_wrapper_lbW_memcore_U->address1 ( memcore_taddr );
        xillybus_wrapper_lbW_memcore_U->d1       ( t_d0 );
        xillybus_wrapper_lbW_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "xillybus_wrapper_lbW.trace");
            // ports
