...
#ifndef _xillybus_wrapper_hbi_H_
#define _xillybus_wrapper_hbi_H_
...
protected:
    ...
    // buffer instances
    xillybus_wrapper_hbi_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(xillybus_wrapper_hbi) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "xillybus_wrapper_hbi_memcore_U_";
            name += szTmp;

            buffer[i] = new xillybus_wrapper_hbi_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "xillybus_wrapper_hbi.trace");
            // ports
